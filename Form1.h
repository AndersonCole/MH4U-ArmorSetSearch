...
#pragma region Members
	...
	private: MH4GASS::NumericUpDownHR^  nudHR;
	private: MH4GASS::NumericUpDownVE^  nudElder;
	...
	private: System::Windows::Forms::ToolStripMenuItem^  mnuAllowExcavatedWeapons;
			...
			for each( Skill^ skill in the_list )
			{
					...
					!!Utility::FindByName( %skill->ability->tags, SkillTag::tags[ filter ]->name ) )
				{
					...
					if( mnuShowRequiredSkillPoints->Checked )
						box->Items->Add( FormatString2( SkillPointsRequiredFormat, skill->name, skill->points_required ) );
					else	
	...
	public:
		...
		Form1(void) : CFG_FILE( L"settings.cfg" ), endl( L"\r\n" ), construction_complete( false ), MAX_LIMIT( 1000 )
		{
			...
			if( !ExcavatedGear::LoadCustom() )
			{
				...
				if( failed )
					MessageBox::Show( L"Failed to load relics file!" );
			}
		...
		void AddSolution( String^ line, const unsigned version )
		{
			...
			const int wep_index = Convert::ToInt32( split[ 0 ] );
			if( wep_index >= 0 && wep_index < ExcavatedGear::weapons.Count )
				solution->weapon = ExcavatedGear::weapons[ wep_index ];
			for( unsigned i = 0; i < 5; ++i )
			{
				const int index = Convert::ToInt32( split[ i + 1 ] );
				if( index < 0 )
					solution->armors.Add( nullptr );
				else solution->armors.Add( Armor::static_armors[ i ][ index ] );
			}
			unsigned num_decorations = 0;
			const unsigned num_torso_decorations = Convert::ToUInt32( split[ 10 ] );
			...
			for( unsigned i = 0; i < num_torso_decorations; ++i )
				solution->body_decorations.Add( Decoration::static_decorations[ Convert::ToUInt32( split[ 12 + i ] ) ] );
			for( unsigned i = 0; i < num_non_torso_decorations; ++i )
				solution->non_body_decorations.Add( Decoration::static_decorations[ Convert::ToUInt32( split[ 12 + num_torso_decorations + i ] ) ] );
			solution->decorations.AddRange( %solution->body_decorations );
			solution->decorations.AddRange( %solution->non_body_decorations );
			
			const unsigned num_skills = Convert::ToUInt32( split[ 11 + num_decorations ] );
			for( unsigned i = 0; i < num_skills; ++i )
			{
				const int index = Convert::ToUInt32( split[ 12 + num_decorations + i ] );
				solution->extra_skills.Add( Skill::static_skills[ index ] );
			}
			unsigned num_pot_skills = 0;
			if( version >= 4 )
			{
				num_pot_skills = Convert::ToUInt32( split[ 12 + num_decorations + num_skills ] );
				for( unsigned i = 0; i < num_pot_skills; ++i )
				{
					const int index = Convert::ToUInt32( split[ 13 + num_decorations + i + num_skills ] );
					solution->potential_extra_skills.Add( Skill::static_skills[ index ] );
				}
			}
			
			const unsigned charm_start = 13 + num_decorations + num_skills + num_pot_skills;
			...
			solution->total_slots_spare = Convert::ToUInt32( split[ 6 ] );
			
			for( unsigned i = 1; i <= 3; ++i )
				solution->slots_spare[ i ] = Convert::ToUInt32( split[ 6 + i ] );
		...
		void LoadConfig( String^ file, const bool lang )
		{
			...
			if( IO::File::Exists( file ) )
			{
				...
				{
					const int temp = Convert::ToInt32( fin.ReadLine() );
					if( lang )
					{
						language = -1;
						if( temp < 0 )
					...
					if( version >= 4 )
					{
						mnuAllowExcavatedArmor->Checked = Convert::ToBoolean( fin.ReadLine() );
						if( version >= 5 )
							mnuAllowExcavatedWeapons->Checked = Convert::ToBoolean( fin.ReadLine() );
					...
					const unsigned num_saved = version >= 8 ? NumSkills : 6;
		...
		void SaveConfig( String^ file )
		{
			...
			fout.WriteLine( mnuAllowExcavatedArmor->Checked );
			fout.WriteLine( mnuAllowExcavatedWeapons->Checked );
			...
			for each( Solution^ solution in all_solutions )
			{
				if( solution->weapon )
				{
					fout.Write( Convert::ToString( solution->weapon->index ) );
					fout.Write( L" " );
				}
				else fout.Write( L"-1 " );
				for( unsigned i = 0; i < 5; ++i )
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->mnuAllowExcavatedArmor = (gcnew System::Windows::Forms::ToolStripMenuItem());
			this->mnuAllowExcavatedWeapons = (gcnew System::Windows::Forms::ToolStripMenuItem());
			...
			this->lblElder->Size = System::Drawing::Size(83, 13);
			this->lblElder->TabIndex = 2;
			this->lblElder->Text = L"Caravan Quests";
			// 
			// lblSlots
			// 
			...
			// 
			// mnuAllowExcavatedArmor
			// 
			this->mnuAllowExcavatedArmor->Checked = true;
			this->mnuAllowExcavatedArmor->CheckOnClick = true;
			this->mnuAllowExcavatedArmor->CheckState = System::Windows::Forms::CheckState::Checked;
			this->mnuAllowExcavatedArmor->Name = L"mnuAllowExcavatedArmor";
			this->mnuAllowExcavatedArmor->Size = System::Drawing::Size(238, 22);
			this->mnuAllowExcavatedArmor->Text = L"Allow E&xcavated Armor";
			// 
			// mnuAllowExcavatedWeapons
			// 
			this->mnuAllowExcavatedWeapons->Checked = true;
			this->mnuAllowExcavatedWeapons->CheckOnClick = true;
			this->mnuAllowExcavatedWeapons->CheckState = System::Windows::Forms::CheckState::Checked;
			this->mnuAllowExcavatedWeapons->Name = L"mnuAllowExcavatedWeapons";
			this->mnuAllowExcavatedWeapons->Size = System::Drawing::Size(238, 22);
			this->mnuAllowExcavatedWeapons->Text = L"Allow Excavated &Weapons";
			// 
			...
			// grpCharms
			// 
			this->grpCharms->Controls->Add(this->btnEquipment);
			...
			// btnEquipment
			// 
			this->btnEquipment->Location = System::Drawing::Point(96, 46);
			this->btnEquipment->Name = L"btnEquipment";
			this->btnEquipment->Size = System::Drawing::Size(89, 23);
			this->btnEquipment->TabIndex = 3;
			this->btnEquipment->Text = L"My &Equipment";
			this->btnEquipment->UseVisualStyleBackColor = true;
			this->btnEquipment->Click += gcnew System::EventHandler(this, &Form1::btnEquipment_Click);
			// 
			// cmbCharmSelect
			// 
...
private: 
	System::Void FormulateQuery( const bool danger, const bool use_gunner_skills, List_t< Skill^ >^ skills )
	{
		...
		query->allow_excavated_armor = mnuAllowExcavatedArmor->Checked;
		query->allow_excavated_weapons = mnuAllowExcavatedWeapons->Checked;
		query->add_extra = mnuSpendSpareSlots->Checked;
		query->allow_guner_helms = mnuAllowGunnerHelms->Checked;
		...
		//query->skills.Sort( gcnew Comparison< Skill^ >( CompareSkillsByName ) );
	...
	System::Void StartSearch()
	{
		...
		if( query->skills.Count > 0 )
		{
			...
			if( cmbCharmSelect->SelectedIndex > 1 ) 
			{
				...
				{
					if( ab == SpecificAbility::auto_guard )
					{
	...
	System::Void UpdateResultString( List_t< Solution^ >^ solutions )
	{
		...
		for each( Solution^ solution in solutions )
		{
			...
			if( solution->weapon )
			{
				sb.AppendLine( solution->weapon->GetName() );
				offset++;
			}
...
#pragma region Worker Thread Stuff
	...
	System::Void backgroundWorker1_DoWork( System::Object^ sender, System::ComponentModel::DoWorkEventArgs^ e )
	{
		try
		{
			...
			if( query->rel_weapons.Count == 0 ) query->rel_weapons.Add( nullptr );
				...
				for( int j = 0; j < body->Count; ++j )
				{
					...
					for( int k = 0; k < arms->Count; ++k )
					{
						for( int l = 0; l < waist->Count; ++l )
						{
							...
							{
								for( int n = 0; n < query->rel_weapons.Count; ++n )
								{
									...
									job->weapon = query->rel_weapons[ n ];
...
#define UpdateMenu( X ) mnu##X->Text = StaticString( X )
		...
		UpdateMenu( AllowExcavatedArmor );
		UpdateMenu( AllowExcavatedWeapons );
...
#undef UpdateMenu
		...
		btnEquipment->Text = StaticString( MyEquipment );
		...
		if( ExcavatedGear::armor )
		{
			for( int i = 0; i < ExcavatedGear::armor->Length; ++i )
			{
				for each( Armor^ a in ExcavatedGear::armor[ i ] )
					a->SetExName( i );
			}
		}
	...
	System::Void contextMenuStrip1_Opening( System::Object^ sender, CancelEventArgs^ e )
	{
		...
		for( int i = 0; i < result_offsets.Count; ++i )
		{
				...
				line < offset + num_armors )
			{
				...
				if( preview_pane->SetData( final_solutions[ i ]->armors ) )
				{
					preview_pane->Show( this );

					preview_pane->Location = Point( cmsSolutions->Location.X + cmsSolutions->Size.Width, cmsSolutions->Location.Y );
				}
	...
	System::Void btnEquipment_Click( System::Object^ sender, System::EventArgs^ e )
	{
		ManageEquipment manager;
		manager.ShowDialog( this );

		for( int i = 0; i < ExcavatedGear::weapons.Count; ++i )
			ExcavatedGear::weapons[ i ]->index = i;

		for( int t = 0; t < ExcavatedGear::armor->Length; ++t )
			for( int i = 0; i < ExcavatedGear::armor[ t ]->Count; ++i )
				ExcavatedGear::armor[ t ][ i ]->index = i;
